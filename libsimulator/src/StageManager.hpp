...
public:
        ...
        std::vector<GenericAgent::ID>& removedAgentsInLastIteration)
    {
        std::unique_ptr<BaseStage> stage = std::visit(
            overloaded{
                ...
                [](const StairDescription& d) -> std::unique_ptr<BaseStage> {
                    return std::make_unique<Stair>(
                        d.position,
                        d.distance,
                        d.length,
                        d.speedFactor,
    ...
    void HandleRemoveAgent(BaseStage::ID stageId, GenericAgent::ID agentId)
    {
        auto* stage = stages.at(stageId).get();
