...
namespace
{
double DesiredSpeedFromAgent(const GenericAgent& agent)
{
    return std::visit(
...
{
    if(distance <= 0.0) {
        throw SimulationError("Stair distance must be > 0.");
    }
    if(length < 0.0) {
    ...
    if(timeStep <= 0.0) {
        throw SimulationError("Stair timeStep must be > 0.");
    }
}

bool Stair::IsCompleted(const GenericAgent& agent)
{
...
Point Stair::Target(const GenericAgent&)
{
    return position;
}

StageProxy Stair::Proxy(Simulation* simulation_)
{
